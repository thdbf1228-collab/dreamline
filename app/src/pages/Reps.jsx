...
export default function Reps() {
  ...
  useEffect(() => {
    supabase.from('reps').select('name, photo_url, group_id, groups(name)').eq('active', true).order('name')
      .then(({ data }) => {
        const list = data || []
        setReps(list)
  ...
  const totalAmount = m ? m._rows.reduce((s, r) => s + (Number(r.display_amount) || 0), 0) : 0
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <select value={sel} onChange={(e) => setSel(e.target.value)}
            ...
            {reps.map((r) => (<option key={r.name} value={r.name}>{r.name}{r.groups?.name ? ` · ${r.groups.name}` : ''}</option>))}
          </select>
        </div>
      </header>
      ...
      {!sel || !m ? null : (
        <div className="grid md:grid-cols-[300px_1fr] gap-4 items-stretch">
          <Card className="p-5 flex flex-col items-center">
            ...
            <div className="mt-3 w-full flex-1 flex items-center">
              <Hexagon data={hex} />
            </div>
          </Card>
          ...
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <KpiCard label="전체 금액" value={won(totalAmount)} sub={`${m._k.total}건`} />
            ...
            <Card className="p-6 flex-1 flex flex-col">
              ...
              {m._k.total === 0 ? (
                ...
                <div className="flex-1 flex flex-col justify-center"><BigFunnel data={f} /></div>
              )}
            </Card>
          </div>
        </div>
...
function Avatar({ url, name }) {
  if (url) return <img src={url} alt={name} className="w-20 h-20 rounded-full object-cover border border-line" />
  return <div className="w-20 h-20 rounded-full bg-brand-soft flex items-center justify-center text-2xl font-bold text-brand">{name?.slice(-2)}</div>
}
